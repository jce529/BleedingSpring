...
/// </summary>
public class WaterStanceMain : SkillBase
{
    [Header("[Water Stance] Main — 참격")]
    ...
    {
        // [U] 참격 스킬의 기본 소모량 설정 (SkillBase 기본값 덮어쓰기)
        hpCostPerStage = new float[] { 2f, 10f, 20f, 40f };
    }
    ...
    private void ExecuteSlash(int stage)
    {
        ...
        float   baseDmg = baseDamage * DamageMult[stage] * GetFinalEffectMultiplier();
        
        // [세계관 준수] 2단계는 HP를 깎지 않고 정화율만 올림
        float   hpDamage = (stage == 2) ? 0f : baseDmg;
        float   corruptionDamage = GetCorruptionDamage(baseDmg);
        ...
        {
            h.GetComponent<IDamageable>()?.TakeDamage(hpDamage, corruptionDamage);
        ...
        string[] stageNames = { "횡베기", "수면 가르기", "폭풍 베기(고압)" };
        string   knockInfo  = stage == 0 ? $" | 넉백: {knockbacks}명" : "";
        Debug.Log($"[WaterMain] {stageNames[stage]} — 적중: {hits.Length}명 | " +
                  $"HP데미지: {hpDamage:F0} | 정화주입: {corruptionDamage:F0}{knockInfo}");
    }
    ...
    private void ExecuteTsunamiSlash()
    {
        ...
        float baseDmg = baseDamage * DamageMult[3] * GetFinalEffectMultiplier();
        float corruptionDmg = GetCorruptionDamage(baseDmg);

        var enemyHits = Physics2D.OverlapBoxAll(center, size, 0f, enemyLayer);
        foreach (var h in enemyHits)
        {
            h.GetComponent<IDamageable>()?.TakeDamage(baseDmg, corruptionDmg);
        }
        ...
        Debug.Log($"[U] 해일참 — 박스: {tsunamiWidth:F0}×{tsunamiHeight:F0} | " +
                  $"적중: {enemyHits.Length}명 | 오염 제거: {contamHits.Length}개");
