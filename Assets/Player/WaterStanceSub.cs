...
/// </summary>
public class WaterStanceSub : SkillBase
{
    [Header("[Water Stance] Sub — 투사체")]
    ...
    {
        // [I] 원거리 스킬의 기본 소모량 설정
        hpCostPerStage = new float[] { 0.5f, 5f, 15f, 30f };
    }
    ...
    protected override IEnumerator ExecuteSkill()
    {
        switch (Stage)
        {
            ...
            case 1:
                FireProjectile(baseDamage * GetFinalEffectMultiplier(), stage1Speed, penetrating: false);
                Debug.Log($"[I] 수압탄 — 속도: {stage1Speed} | 데미지: {baseDamage * GetFinalEffectMultiplier():F0}");
                break;

            case 2:
                FireProjectile(baseDamage * 1.3f * GetFinalEffectMultiplier(), stage2Speed, penetrating: false);
                Debug.Log($"[I] 고속 수압탄 — 속도: {stage2Speed} | 데미지: {baseDamage * 1.3f * GetFinalEffectMultiplier():F0}");
                break;
    ...
    private void FireLaser()
    {
        ...
        float laserDmg = sniperDamage * GetFinalEffectMultiplier();
        ...
        Debug.Log($"[I] 혈연의 쐐기 — 레이저 사거리: {sniperRange:F0} | " +
                  $"관통 적중: {hits.Length}명 | 데미지: {laserDmg:F0}");
    }
